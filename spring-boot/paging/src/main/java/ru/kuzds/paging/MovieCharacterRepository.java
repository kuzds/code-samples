...
interface MovieCharacterRepository extends CrudRepository<MovieCharacter, Long> {
    ...
    Slice<MovieCharacter> findByMovieCustom(@Param("movie") String movieName, Pageable pageable);

    @Query("select c from MovieCharacter c where c.movie = :movie")
    List<MovieCharacter> findByMovieSorted(@Param("movie") String movieName, Sort sort);
}
